...
XDataTestExt = data.frame(type=factor(rep("pa", nrow(XDataTest)), levels(XData$type)), obs=0)
XDataTest = cbind(XDataTestExt, XDataTest)
...
predBase = matrix(colMeans(Y_pa), nrow=nrow(XDataTest), ncol=ncol(Y_pa), byrow=TRUE)
indCommon = order(colSums(Y_pa), decreasing=TRUE)[1:m$ns]
